...
module MixinBot
  ...
  class MixAddress
    attr_accessor :version, :uuid_members, :xin_members, :threshold, :address
    ...
    def decode
      ...
      if data[3...-4].length == members_count * UUID_ADDRESS_LENGTH
        uuid_members = data[3...-4].chars.each_slice(UUID_ADDRESS_LENGTH).map(&:join)
        self.uuid_members = uuid_members.map(&->(member) { MixinBot::UUID.new(raw: member).unpacked })
        self.xin_members = []
      else
        xin_members = data[3...-4].chars.each_slice(MAIN_ADDRESS_LENGTH).map(&:join)
